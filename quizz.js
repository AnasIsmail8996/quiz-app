...
var questions = [
  ...
  var next = document.getElementById('nextQuestions');
     next && next.addEventListener('click', nextQuestions);
  ...
  function nextQuestions() {
    ...
    for (var i = 0; i < options.length; i++) {
      if (options[i].checked) {
       ...
       var userAnswer = (questions[index - 1][`option${selectedValue}`]);
  ...
  if(userAnswer === correctAnswer){
    ...
    if (index > questions.length - 1) {
      ...
      Swal.fire({
        ...
        option3.innerText = questions[index].option1;
        option2.innerText = questions[index].option2;
        option1.innerText = questions[index].option3;
        index++;
